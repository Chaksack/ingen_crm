...
async function onSubmit(event: Event) {
  ...
  try {
    await $fetch('/api/financial-institutions', { method: 'POST', body: form })
    toast.success('Financial institution created')
    emit('created')
  }
  catch (err: any) {
    toast.error(err?.data?.statusMessage || 'Failed to create financial institution')
  }
  finally {
...
<template>
  <form class="grid gap-4 px-4" @submit="onSubmit">
    <div class="grid gap-2">
      ...
      <Input id="institutionName" v-model="form.institutionName" :disabled="isLoading" />
    </div>
    <div class="grid gap-2">
    ...
    <div class="grid grid-cols-2 gap-4">
      <div class="grid gap-2">
        ...
        <Input id="institutionType" v-model="form.institutionType" placeholder="e.g. Commercial Bank" :disabled="isLoading" />
      </div>
      <div class="grid gap-2">
