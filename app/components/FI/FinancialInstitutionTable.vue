...
const { data: institutions, pending: isLoading, error, refresh } = useFetch<FinancialInstitution[]>('/api/financial-institutions')

const searchQuery = ref('')
const statusFilter = ref('all')
const currentPage = ref(1)
const itemsPerPage = ref(10)
const selectedInstitution = ref<FinancialInstitution | null>(null)
...
const filteredInstitutions = computed(() => {
  ...
  if (statusFilter.value !== 'all')
    filtered = filtered.filter(institution => institution.status === statusFilter.value)

  return filtered
})

const totalPages = computed(() => Math.ceil(filteredInstitutions.value.length / itemsPerPage.value))

const paginatedInstitutions = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage.value
  return filteredInstitutions.value.slice(start, start + itemsPerPage.value)
})
...
function viewInstitution(institution: FinancialInstitution) {
  ...
  <div>
    <main class="@container/main flex flex-1 flex-col gap-4 md:gap-8">
      <Alert v-if="error" variant="destructive">
        ...
        <AlertDescription>Failed to load financial institutions.</AlertDescription>
      </Alert>
        ...
        <CardHeader>
          <div class="flex flex-col gap-4">
            ...
            <div class="text-sm text-muted-foreground">
              Showing {{ paginatedInstitutions.length }} of {{ filteredInstitutions.length }} results
              <span v-if="searchQuery || statusFilter !== 'all'">
                (filtered from {{ institutions?.length ?? 0 }} total)
              </span>
            </div>
          </div>
        </CardHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead v-if="columnVisibility.institutionName">
                    Institution Name
                  </TableHead>
              ...
              <TableBody v-if="isLoading">
                <TableRow v-for="i in itemsPerPage" :key="`skeleton-${i}`">
                  ...
                  <TableCell v-if="columnVisibility.institutionName">
              ...
              <TableBody v-else-if="paginatedInstitutions.length > 0">
                <TableRow v-for="institution in paginatedInstitutions" :key="institution.id">
                  <TableCell v-if="columnVisibility.avatar">
                    <Avatar class="relative overflow-visible">
                      <AvatarImage class="rounded-full" :src="institution.avatar || ''" alt="Institution Avatar" />
                      <AvatarFallback class="text-white">
                        {{ institution.institutionName.substring(0, 2).toUpperCase() }}
                      </AvatarFallback>
                      <span
                        :class="institution.status === 'active' ? 'bg-green-500' : 'bg-red-500'"
                  ...
                  <TableCell v-if="columnVisibility.actions">
                    <Button size="sm" variant="default" @click="viewInstitution(institution)">
                      View
                    </Button>
                  </TableCell>
                </TableRow>
                    ...
                    <div class="flex flex-col items-center justify-center py-12">
                      ...
                      <p class="text-sm max-w-sm text-muted-foreground">
                        {{ searchQuery || statusFilter !== 'all'
                          ...
                          : 'No financial institutions yet. Get started by creating one.' }}
                      </p>
                      <Button
    ...
    <Sheet v-model:open="isSheetOpen">
      <SheetContent side="right" class="w-full sm:max-w-2xl overflow-y-auto">
        <SheetHeader>
          <SheetTitle class="flex items-center gap-3">
            <Avatar class="h-12 w-12">
              ...
              <AvatarFallback>
                {{ selectedInstitution?.institutionName ? selectedInstitution.institutionName.substring(0, 2).toUpperCase() : 'FI' }}
              </AvatarFallback>
            </Avatar>
            <div>
        ...
        <div v-if="selectedInstitution" class="mt-6 space-y-6 px-4">
          <div class="flex items-center gap-2">
          ...
          <Card>
            ...
            <CardContent class="space-y-3">
              <div class="grid grid-cols-3 gap-2">
                ...
                <span class="col-span-2 text-sm font-medium">{{ selectedInstitution.institutionType || 'N/A' }}</span>
              </div>
              <div class="grid grid-cols-3 gap-2">
                <span class="text-sm text-muted-foreground">Registration Number:</span>
                <span class="col-span-2 text-sm font-medium">{{ selectedInstitution.registrationNumber || 'N/A' }}</span>
              </div>
              <div class="grid grid-cols-3 gap-2">
                <span class="text-sm text-muted-foreground">Founded:</span>
                <span class="col-span-2 text-sm font-medium">{{ selectedInstitution.foundedDate || 'N/A' }}</span>
              </div>
              <div class="grid grid-cols-3 gap-2">
