        ...
        <Sheet>
          <SheetTrigger as-child>
            <Button variant="outline" class="">
              <Icon name="i-lucide-loader-pinwheel" class="w-4 h-4" /> Ask Ai
            </Button>
          </SheetTrigger>
...
<DropdownMenu>
  ...
  <template v-if="messages.length > 0">
    <template v-for="(msg, i) in messages" :key="i">
      ...
      <div class="flex-1 justify-center">
        <p class="text-xs mx-2 text-muted-foreground mb-2">subject: {{ msg.subject }}</p>
        <p class="text-xs mx-2 text-muted-foreground mb-4">{{ msg.content }}</p>
      </div>
    ...
    <DropdownMenuItem>
      <NuxtLink to="/email" class="w-full text-center">View All Messages</NuxtLink>
    </DropdownMenuItem>
  </template>
  ...
  <template v-if="notifications.length > 0">
    <template v-for="(item, i) in notifications" :key="i">
      ...
      <p class="text-xs mx-2 text-muted-foreground mb-4">{{ item.description }}</p>
