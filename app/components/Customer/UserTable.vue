...
const fetchCustomers = async () => {
  ...
  try {
    ...
    const response = await nuxtApp.$axios.get('http://localhost:8080/api/customers', {
      withCredentials: true,
    })
    customers.value = response.data
  } catch (err) {
    error.value = 'Failed to load customer data.'
    toast.error('Error fetching customers: ' + (err as Error).message)
  } finally {
              ...
              <div class="flex gap-2 flex-wrap">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuCheckboxItem v-model:checked="columnVisibility.firstName">
                      First Name
                    </DropdownMenuCheckboxItem>
                    <DropdownMenuCheckboxItem v-model:checked="columnVisibility.lastName">
                      Last Name
                    </DropdownMenuCheckboxItem>
                    <DropdownMenuCheckboxItem v-model:checked="columnVisibility.age">
                      Age
                    </DropdownMenuCheckboxItem>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead v-if="columnVisibility.firstName">First Name</TableHead>
                  <TableHead v-if="columnVisibility.lastName">Last Name</TableHead>
                  <TableHead v-if="columnVisibility.age">Age</TableHead>
              ...
              <TableBody v-if="isLoading">
                <TableRow v-for="i in itemsPerPage" :key="`skeleton-${i}`">
                  ...
                  <TableCell v-if="columnVisibility.firstName"><Skeleton class="h-4 w-24" /></TableCell>
                  <TableCell v-if="columnVisibility.lastName"><Skeleton class="h-4 w-24" /></TableCell>
                  <TableCell v-if="columnVisibility.age"><Skeleton class="h-4 w-8" /></TableCell>
              ...
              <!-- Data State -->
              <TableBody v-else-if="paginatedCustomers && paginatedCustomers.length > 0">
                <TableRow v-for="customer in paginatedCustomers" :key="customer.ID">
                  <TableCell v-if="columnVisibility.id">{{ customer.ID || 'N/A' }}</TableCell>
                  <TableCell v-if="columnVisibility.avatar">
                    <Avatar class="relative overflow-visible">
                      <AvatarImage class="rounded-full" :src="customer.avatar || ''" alt="Customer Avatar" />
                      <AvatarFallback class="text-white">
                        {{ customer.first_name && customer.last_name ? `${customer.first_name[0]}${customer.last_name[0]}` : '' }}
                      </AvatarFallback>
                      <span
                        v-if="customer"
                  ...
                  <TableCell v-if="columnVisibility.firstName">{{ customer.first_name || 'N/A' }}</TableCell>
                  <TableCell v-if="columnVisibility.lastName">{{ customer.last_name || 'N/A' }}</TableCell>
                  <TableCell v-if="columnVisibility.age">{{ customer.age || 'N/A' }}</TableCell>
                  <TableCell v-if="columnVisibility.email">{{ customer.email || 'N/A' }}</TableCell>
                  <TableCell v-if="columnVisibility.phoneNumber">{{ customer.phone_number || 'N/A' }}</TableCell>
                  ...
                  <TableCell v-if="columnVisibility.actions">
                    <NuxtLink :to="`/company/view/${customer.ID}`">
                    ...
                    <div class="flex flex-col items-center justify-center py-12">
                      ...
                      <p class="text-sm max-w-sm">
                        {{ searchQuery || statusFilter !== 'all' 
                          ...
                          : 'No customer data available. Get started by creating your first customer.' 
                        }}
                      </p>
                      <Button 
